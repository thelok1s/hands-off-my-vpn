...
Приложения уже давно следят за активным VPN-соединением и просят его отключить. Однако, в апреле 2026 года было [проведено исследование](https://files.rks.global/russian_apps_search_for_vpn_ru.pdf), которое показало, что многие популярные российские приложения не просто смотрят IP, а различными методами исследуют все устройство ради обнаружения VPN соединений и программ, нарушая приватность пользователей и передавая данные на свои сервера, откуда они могут попасть в руки третьих лиц.     

Приложения эксплуатируют слабую защиту приватности системы Android в области сетевых подключений, чтобы ограничивать подключение с VPN (даже если активно туннелирование), компрометировать VPN-сервера (как MAX), либо собирать излишнее количество данных, при этом защищаясь от исследования механизмов сбора, но не защищая их от утечки.
...
2. Установите LSposed-модуль [Hide-My-Applist](https://github.com/dr-tsng/hide-my-applist) (скорее всего, он уже у вас есть). По своему усмотрению скройте только VPN и Root приложения (черный список), либо включите режим белого списка (хехе) – некоторые приложения могут перестать открываться.
3. Проверьте работоспособность конфигурации применив модуль к [VPN Detector](https://github.com/cherepavel/VPN-Detector)
...
## Благодарности
Спасибо [VPN Detector](https://github.com/cherepavel/VPN-Detector) за реализацию методов обнаружения для тестирования!

## Известные проблемы
На данный момент Альфа-банк, Т-банк и ВТБ не работают, если модуль активен для них. Для части приложений тестирование еще не проводилось
...
### Additional:
1. Enable split tunneling and routing via geoip in your VPN client.
2. Install LSposed-module [Hide-My-Applist](https://github.com/dr-tsng/hide-my-applist) (if you don't have it already). Add target apps to blacklist (hide vpn clients) or whitelist (apps might stop working)
3. Check your setup with [VPN Detector](https://github.com/cherepavel/VPN-Detector)
...
## Credits
Thanks to [VPN Detector](https://github.com/cherepavel/VPN-Detector) for implementing vpn detection methods for testing!
